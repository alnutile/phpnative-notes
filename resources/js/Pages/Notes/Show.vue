...
const resetTimer = () => {
    ...
    inactivityTime.value = setTimeout(save, 300000); // 300000 ms = 5 minutes
};
...
onBeforeUnmount(() => {
    clearTimeout(inactivityTime.value); // Clear the timer
...
<template>
    <AppLayout>
        ...
        <div>
            <div class="max-w-full mx-auto dark:text-gray-300">
                <div class="flex justify-between gap-4 items-center">
                    <div class="w-4/6">
                        <input class="bg-slate-900 border-0 text-lg w-full" v-model="form.title" />
                    </div>
                    <div class="flex items-center">
                        <button
                            v-shortkey="['ctrl', 's']" @shortkey="save()"
                            @click="save" class="text-pink-600">save</button>
                    </div>
                </div>

                <div>
                    <Tagger :note="note.data"/>
                </div>
            </div>
                                ...
                                <div class="flex flex-wrap items-center divide-gray-200 sm:divide-x sm:rtl:divide-x-reverse dark:divide-gray-600">
                                    <div class="flex items-center space-x-1 rtl:space-x-reverse sm:pe-4">
                                        ...
                                        <button
                                            ...
                                            type="button" class="p-2 text-gray-500 rounded cursor-pointer hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600">
                                            <svg class="w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 20">
            ...
            <h2 class="mt-10 text-2xl text-gray-400">Files:</h2>
            <div class="grid grid-cols-2 gap-4">
                <div
                    v-for="file in note.data.files" :key="file.id" >
                    <div class="mb-2">
                        <a
                            class="underline"
                            :href="file.link">Name: {{file.name}}</a>
                    </div>
                    <img
        ...
        <dialog id="my_modal_1" class="modal" :class="{ 'modal-open' : showFileUpload }">
            <div class="modal-box">
                <h3 class="font-bold text-lg">Upload files</h3>
                <div class="modal-action">
                    <Files :note="note.data"/>
                </div>
                <div class="flex justify-end">
                    <button class="btn" type="button" @click="showFileUpload = false">Close</button>
                </div>
